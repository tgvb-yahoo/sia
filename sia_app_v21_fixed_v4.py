...
def normalize_01(x, lo, hi):
    ...
    if hi is not None and lo is not None and hi != lo:
        return np.clip((x_arr - lo) / (hi - lo), 0, 1)
    # Degenerate range -> zeros like x (preserve shape/index)
    try:
...
def to_series(x, index):
    ...
    return pd.Series(np.nan_to_num(x, nan=0.0), index=index)
...
# Coerce HI components to Series to avoid scalar collapse
sW_b = to_series(sW_b, df.index)
sT_b = to_series(sT_b, df.index)
sK_b = to_series(sK_b, df.index)
sA_b = to_series(sA_b, df.index)
sW_a = to_series(sW_a, df.index)
sT_a = to_series(sT_a, df.index)
sK_a = to_series(sK_a, df.index)
sA_a = to_series(sA_a, df.index)


HI_b = (wW*safe_num(sW_b) + wT*safe_num(sT_b) + wK*safe_num(sK_b) + wA*safe_num(sA_b)) / den
HI_a = (wW*safe_num(sW_a) + wT*safe_num(sT_a) + wK*safe_num(sK_a) + wA*safe_num(sA_a)) / den
